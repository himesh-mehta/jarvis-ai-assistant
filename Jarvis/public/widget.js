(function () {
  // ── Get config (supports both script tag AND window.JARVIS_CONFIG) ──
  const script = document.currentScript;
  const global = window.JARVIS_CONFIG || {};

  const apiKey  = (script && script.getAttribute('data-key'))
                  || global.key         || '';
  const name    = (script && script.getAttribute('data-name'))
                  || global.name        || 'JARVIS';
  const color   = (script && script.getAttribute('data-color'))
                  || global.color       || '#00E5FF';
  const avatar  = (script && script.getAttribute('data-avatar'))
                  || global.avatar      || '🤖';
  const welcome = (script && script.getAttribute('data-welcome'))
                  || global.welcome     || '';
  const prompt  = (script && script.getAttribute('data-prompt'))
                  || global.prompt      || '';
  ...
  bubble.addEventListener('click', () => {
    ...
    if (isOpen) {
      ...
      bubble.innerHTML = `<span id="jarvis-bubble-icon">${avatar}</span>`;
    }
  });
